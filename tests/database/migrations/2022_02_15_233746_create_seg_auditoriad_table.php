...
class CreateSegAuditoriadTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('seg_auditoriad', function (Blueprint $table) {
            $table->integer('auditoriaDId')->primary();
            $table->integer('auditoriaId');
            $table->string('comentarios', 100);
            
            $table->foreign('auditoriaId', 'fk_SEG_AUDITORIAD_SEG_AUDITORIA1')->references('auditoriaId')->on('seg_auditoria')->onDelete('restrict')->onUpdate('restrict');
        });
    }
